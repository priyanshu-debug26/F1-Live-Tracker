...
function App() {
  ...
  useEffect(() => {
    ...
    if (!path || !dot || !svg) {
      console.error('Missing elements:', { path, dot, svg });
      return;
    }
    ...
    function moveDot() {
      ...
      const trackWrapperRect = svg.parentElement.getBoundingClientRect();

      const scaleX = svgRect.width / 500;
      const scaleY = svgRect.height / 500;

      // Get offset - but SVG might have centering
      let offsetX = svgRect.left - trackWrapperRect.left;
      let offsetY = svgRect.top - trackWrapperRect.top;

      dot.style.left = `${point.x * scaleX + offsetX}px`;
      dot.style.top = `${point.y * scaleY + offsetY}px`;
      ...
      <main>
        ...
        <div className='track'>
          <div className="track-wrapper">
            <div ref={dotRef} className='driver-dot'></div>
            <svg
              ref={svgRef}
              className='track-map'
            ...
            >
              ...
              <path
                d="m327.823 380.041-39.147 8.217-2.307-9.73 39.147-8.217z"
                fill="#000"
              />

              <path
                d="m345.358 369.575-2.394 3.774-2.395 3.773-3.34-3.005-3.341-3.005"
                fill="none"
                stroke="#000"
                strokeWidth="2"
